...
**Phase:** Week 2 in progress — Prompt 5 (bulk add paste mode + Stakeholders screen) code-complete and committed (`c40b61f`). Scaffold (Prompt 1) + full schema (Prompts 2/4) + magic-link auth (Prompt 3) + cap-table view (Prompt 4) complete; authenticated `/cap-table` and `/stakeholders` both verified rendering the seeded Acme data via a real local sign-in. **Prompt 6 (Denmark country pack) is next.**
...
**Next action:** Start **Prompt 6 — Denmark country pack** (`lib/packs/dk/`, instrument catalog + validation rules + jurisdiction-aware labels per `docs/03_country_packs_dk_no.md`). Still pending (user-side): set Supabase **Authentication → URL Configuration → Site URL** (+ Redirect URLs) to `https://freecapt.vercel.app` so real emailed magic links resolve in prod (without this, prod sign-in links still point at localhost). **Done:** legacy `service_role` JWT disabled; stray `nr-del/freecapt-vercel` repo deleted.

**Note on "nothing to test":** the public root `/` is the intentional "Coming soon" marketing placeholder. The product (`/cap-table`, `/stakeholders`) lives behind the magic-link auth wall — reachable only after sign-in at `/sign-in`. It is NOT linked from the landing page.
