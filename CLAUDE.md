...
**Next action:** Start **Prompt 6 — Denmark country pack** (`lib/packs/dk/`, instrument catalog + validation rules + jurisdiction-aware labels per `docs/03_country_packs_dk_no.md`). Still pending (user-side, optional): set Supabase **Authentication → URL Configuration → Site URL** (+ Redirect URLs) to `https://freecapt.vercel.app` so real emailed magic links resolve in prod; disable the legacy `service_role` JWT; delete the stray `nr-del/freecapt-vercel` repo.
