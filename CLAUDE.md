...
**Phase:** Week 2 complete — Prompt 5 (bulk add + Stakeholders, `c40b61f`) and Prompt 6 (Denmark country pack, `96e0b85`) both shipped. Scaffold (Prompt 1) + full schema (Prompts 2/4) + magic-link auth (Prompt 3) + cap-table view (Prompt 4) complete. **Prompt 7 (NO/UK/US packs + jurisdiction-aware i18n + basic Simulator + Transactions screen + New Grant modal) is next.**
...
**Next action:** Start **Prompt 7 — Norway, UK, US packs** (Week 3, Day 1): add NO/UK/US country packs following the DK pattern, jurisdiction-aware UI labels via `next-intl`, a basic Simulator, the Transactions screen, and the New Grant modal (wire the DK § 7P validator into the grant flow). **All user-side setup is done:** Supabase Site URL set + prod sign-in works; legacy `service_role` JWT disabled; stray `nr-del/freecapt-vercel` repo deleted.
