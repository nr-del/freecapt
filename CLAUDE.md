...
**Note on the magic-link email (POSTPONED, user-side):** sign-in currently fails with Supabase's `429 over_email_send_rate_limit` because the built-in email sender (`noreply@mail.app.supabase.io`) is capped ~2–3/hour. Fix = configure custom Resend SMTP in Supabase → Auth → Emails → SMTP Settings (Host `smtp.resend.com`, Port 465, User `resend`, Password = `RESEND_API_KEY`, sender on a Resend-verified domain) + raise the email rate limit + confirm Redirect URLs allowlist includes `https://freecapt.vercel.app/**`. The `/sign-in` page now surfaces the rate-limit error in a visible alert (`friendlyAuthError`) instead of silently resetting.
