...
**Week 7–8 — shareholder-register PDF export (done, NOT pushed):** the auto register slot now produces a real download. `pdf-lib` (pure-TS, no native binaries, no React peer — safe in serverless) added. `lib/exports/register.ts` — `buildShareholderRegister(company)` computes the statutory register straight from the cap table (equity-unit holders only; options/convertibles excluded), per-holder shares + % of issued + nominal value (shares×parValue), sorted by shares desc. `lib/exports/register-pdf.ts` — `renderRegisterPdf(reg)` lays out an A4 PDF by hand (identity header with legal name/registry id/jurisdiction + local title e.g. Ejerbog, one-row-per-holder table with pagination + repeating header, totals row, per-page footer with UTC generation date + page numbers; PDF metadata Title/Author/Producer set). `app/(app)/data-room/register/route.ts` — auth-walled GET (route handlers aren't wrapped by the (app) layout, so it re-checks the Supabase session), `force-dynamic`, streams `application/pdf` as an attachment named `<legal-name-slug>-shareholder-register.pdf`. Data room auto slot renders a "PDF" download link to it. tsc + build green (`/data-room/register` is a ƒ dynamic route). Currency-aware (par-value currency) and always current — no stored file. **Polish (done, NOT pushed):** register now consolidates to one row per (holder, share class) and orders by class then shares (a register records holdings, not issuances); a "By class" roll-up line prints when >1 class. Data room documents show a "Signed" pill when `documents.signed`.

**Week 9–10 — Round modeling (§5.25, done, NOT pushed):** the multi-investor allocation workflow that extends the basic simulator. `lib/simulate/round-model.ts` — pure engine on top of `simulateRound`: a modeled round IS a priced round whose `roundSize` = the sum of what's actually allocated, so the dilution/SAFE/pool math is delegated and the single "new investor" line is split into one row per investor (`allocated ÷ pps` shares each); a one-investor `fixed` model equals the basic simulator exactly. Commitment shapes `fixed | range | up_to`, priorities `must_include | target | optional | fill_the_gap`, optional `minCheck` + `proRataPct` (entitlement = ownership × round size). `modelRound()` returns a reusable `SimResult` (`scenario`) + an `AllocationRow[]` (requested/pro-rata/allocated, % of round, % of post, status `ok|cut|below_min|over_max`) + a `RoundValidation` (exact/under/over + per-investor messages). `suggestAllocation()` — deterministic greedy solver (v1 stretch): seeds must-include floors, tops toward maxes in priority order, gap-fillers absorb the remainder. `components/freecapt/round-model-view.tsx` — client workflow: round terms, investor editor (add/remove rows, shape + priority + manual allocation), "Suggest allocation" button, color-coded validation banner, allocation table, and the resulting cap table via `ScenarioView`. Wired into `/simulate` as the now-functional "Round modeling" tab (was a paywall stub; the old multi-round paywall dialog removed). `lib/simulate/round-model.test.ts` — 12 tests (shape bounds, manual allocation matches the basic simulator, per-investor row split, 100% sum, over/under/over-max validation, pro-rata math, greedy solver fills + scales + guarantees must-include). **27 tests total now pass.** tsc + lint + build all green (`/simulate` still ƒ dynamic).

**Week 9–10 — Term-sheet draft (§5.25, PDF, done, NOT pushed):** the jurisdiction-aware non-binding term-sheet draft on top of the round model. `lib/exports/term-sheet.ts` — pure `buildTermSheet(company, pack, terms, model)` → a renderer-agnostic `TermSheet` (identity, economics, `investors` filtered to allocated>0 + sorted desc, `keyTerms`, fullyAllocated, disclaimer). Jurisdiction-aware governing law (`GOVERNING_LAW`: dk→Danish law, no→Norwegian, uk→laws of England and Wales, us→laws of the State of Delaware…) and a SAFE-conversion key term inserted only when SAFEs convert. Uses `pack.equityUnitNoun(entityType)` so the unit noun is local (shares/anparter/aktier). `lib/exports/term-sheet-pdf.ts` — server-only `renderTermSheetPdf(ts)` via pdf-lib, mirrors the register-PDF house style (FreeCapT wordmark, emerald/slate ink, per-page footer): header → identity → round-terms two-column grid → allocation table (paginated, name/amount/%round/units/%post) → totals → key terms (word-wrapped) → signature lines → non-binding disclaimer footer. `app/(app)/simulate/term-sheet/route.ts` — auth-walled POST (route handlers aren't wrapped by the (app) layout) because round terms + allocation live in client state; the cap table is loaded **server-side** (`loadSimData`, extracted to `lib/simulate/load.ts` and shared with the simulate page) so the price/post-money math is authoritative — the client only supplies terms + per-investor allocation. zod-validated body, streams `<slug>-term-sheet.pdf` (`Cache-Control: no-store`). Download button wired into `round-model-view.tsx` (disabled until something is allocated, toasts on error). `lib/exports/term-sheet.test.ts` — 5 tests (economics carry through, allocated investors listed largest-first with share counts, zero-allocation dropped, Delaware governing law, SAFE term only when SAFEs convert). tsc + lint + build all green (`/simulate/term-sheet` is a ƒ dynamic route).

**First-run onboarding (done, NOT pushed):** a signed-in user with no company is now walked through creating their company + profile before they can reach the product. `lib/db/queries.ts` — `getActiveCompany()` is now **membership-scoped** (resolves the signed-in user's most-recent active company via `memberships`, not "any Acme"); split into `getCompanyForEmail(email)` (the membership join) + a session wrapper, plus `emailHoldsEquity(email)` (does this email hold a stakeholder grant anywhere). `app/(app)/layout.tsx` — after the auth wall, a **company guard**: no company → redirect to `/onboarding` (brand-new) or `/portfolio` (holds equity but no admin company). `app/api/auth/callback/route.ts` — a brand-new user (no company, no equity) now lands on `/onboarding` instead of `/cap-table`. `proxy.ts` — `/onboarding` added to `NON_LOCALIZED_PREFIXES` (else next-intl 404s it like the data-room bug did). `app/onboarding/{layout,page,onboarding-client}.tsx` + `actions.ts` — a minimal-chrome first-run form (own `<html>`/`<body>`, no product nav): **Profile** (full name, language, time zone) + **Company** (jurisdiction, entity type, legal/display name, currency, registry id, incorporation date, authorized shares), all pack-seeded and re-seeded when the jurisdiction changes. `page.tsx` maps `AVAILABLE_PACKS` → serializable `JurisdictionOption[]` (functions/RegExps can't cross the server/client boundary); **onboarding is restricted to the four built packs dk/no/uk/us**. `completeOnboarding` (zod-validated server action) upserts the account profile, inserts the company (pack-derived jurisdiction/currency/packVersion, `dataRegion` us→us-east else eu-fra), an admin membership, and a free subscription in one transaction; idempotent (existing company → straight to `/cap-table`); redirects to `/cap-table` on success. **No schema migration needed** (all fields already existed). `lib/db/seed.ts` — because `getActiveCompany` is now membership-scoped, the seeded Acme is invisible until a real account is linked: set `SEED_OWNER_EMAIL=you@example.com pnpm db:seed` to grant your magic-link account an admin membership on the demo company (the wipe block now also clears `memberships`). tsc + lint + build all green (`/onboarding` is a ƒ dynamic route).

**Word (.docx) term-sheet export (done, NOT pushed):** the spec's "PDF + Word" is complete. `docx` 9.7.1 added. `lib/exports/term-sheet-docx.ts` — server-only `renderTermSheetDocx(ts)` builds the same `TermSheet` shape into a Word doc with the `docx` package, mirroring the PDF's content + house palette (wordmark, emerald/slate, hairlines): identity header → round-terms label/value table → allocation table (header + totals row) → key terms → signature-line table → disclaimer footer. The POST route now takes a `format: "pdf" | "docx"` (zod `.default("pdf")`) and dispatches renderer + Content-Type + filename extension accordingly; the round-model view has split **PDF** / **Word** download buttons (independent per-format loading state). `lib/exports/term-sheet.test.ts` gained a docx smoke test (`vi.mock("server-only")`; asserts the output is a ZIP — `PK` magic — and non-trivial size). **33 tests total pass.** **Still TODO for Week 9–10:** the generate-from-template engine (SHA/founder-agreement/board books per pack), and Stripe paywall enforcement (no Stripe account wired yet — round modeling + term-sheet draft are currently open for beta).

**Tests + lint cleanup (done):** first test runner added — **vitest** (`pnpm test` = `vitest run`, `pnpm test:watch`; `vitest.config.ts`, node env, `@` alias, picks up `lib/**/*.test.ts`). Unit tests for the two pure-logic modules: `lib/vesting/compute.test.ts` (7) + `lib/simulate/engine.test.ts` (8) = 15 passing. Writing the vesting tests surfaced + fixed a real bug: pre-cliff `nextVestQty` reported a single period (e.g. 1,000) instead of the whole cliff jump (12/48 = 12,000) that actually lands on `nextVestDate` — `computeVesting` now sums all upcoming periods sharing the next vest date. Lint is now clean (was 4 errors): removed a dead `pageNo` in register-pdf, converted `tailwind.config.ts` plugin `require()`s to ESM imports, and switched the contact page's internal `/status` link from `<a>` to the locale-aware `Link`. When adding pure logic, add a sibling `*.test.ts`; DB/React modules are not yet covered.
